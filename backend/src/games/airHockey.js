...
// COORDINATE SPACE CONTRACT[cite: 14]:
...
// input — only when it is P2. The server doesn't need to know or
...
export default class AirHockeyGame {
    ...
    startGameLoop() {
        ...
        // 60 FPS authoritative physics loop
        this.gameInterval = setInterval(() => {
            if (this.state.status !== 'playing') return;
            this.updatePhysics();
        }, 1000 / 60);

        // 30 FPS network broadcast
    ...
    // ARCHITECTURE RULE 5: Play Again Flow
    handleRematch() {
        this.resetGameState();
        this.startCountdown();
    }
