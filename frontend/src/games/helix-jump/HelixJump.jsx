  ...
  const gameLoop = useCallback(() => {
    ...
    if (canvas) {
      ...
      const drawChunk = (y, chunk) => {
        ...
        // Top Face (drawn as an arc polygon)
        ctx.fillStyle = fill;
        ctx.beginPath();
        for(let i=0; i<=steps; i++) {
          let a = start + i * step;
          ctx.lineTo(160 + R_OUT * Math.cos(a), screenY + TILT * Math.sin(a));
        }
        for(let i=steps; i>=0; i--) {
          let a = start + i * step;
          ctx.lineTo(160 + R_IN * Math.cos(a), screenY + TILT * Math.sin(a));
        }
        ctx.closePath();
        ctx.fill();
        ctx.stroke();

        // 3D Thick Side Lip (Only visible on front pieces)
        if (isFront) {
          ...
          for(let i=steps; i>=0; i--) {
            let a = start + i * step;
            ctx.lineTo(160 + R_OUT * Math.cos(a), screenY + TILT * Math.sin(a) + PLATFORM_THICKNESS);
          }
          ctx.closePath();
          ctx.fill();
          ctx.stroke();
      ...
      <div className="fixed inset-0 pointer-events-none z-40 flex items-center justify-center">
        {popups.map(popup => (
          <div key={popup.id} className="absolute animate-float mt-32">
            <span className={`font-black uppercase text-4xl drop-shadow-[3px_3px_0_#000] 
              ${popup.type === 'combo' ? 'text-[#ef476f]' : 'text-[#06d6a0]'}`}>
              {popup.text}
            </span>
          </div>
        ))}
